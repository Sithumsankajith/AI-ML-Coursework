...
for targetUser = userRange_min:userRange_max
  ...
  assert(sum(yTrain == 0) == (numUsers - 2) * trainSamplesPerImposter);
...
for imposterUser = 1:numUsers
    if imposterUser ~= targetUser
        ...
        imposterData = userData(imposterUser).testFeatures;
        availableSamples = size(imposterData, 1);
        wantedSamples = testSamplesPerImposter;
        
        % 1. Find the real number of samples we can take
        samplesToTake = min(availableSamples, wantedSamples);
        
        % Only proceed if this imposter has samples
        if samplesToTake > 0
            % 2. Select 'samplesToTake' from the 'availableSamples'
            selectedIdx = randperm(availableSamples, samplesToTake);
            testImposterFeatures = [testImposterFeatures; imposterData(selectedIdx, :)];
            
            % 3. Use 'samplesToTake' for the labels (fixes bug 2)
            testImposterLabels = [testImposterLabels; zeros(samplesToTake, 1)];
            testUserLabels = [testUserLabels; ones(samplesToTake, 1) * imposterUser];
        end
        % --- END: Corrected Code ---
